...
#class TestAirline(unittest.TestCase):
    ...
    #def test_calc_overbooked(self):
        #q = 0.047
        #p = 1 - q
        #n = 73
        #k = 71
        #binomial_coefficient = factorial(n) / (factorial(k) * factorial(abs(n-k)))
        #probability = binomial_coefficient * (p ** n) * ((1 - p) ** (n - k))
        #self.assertEqual(airline.calc_overbooked
...
if __name__ == "__main__":
    #TODO: move to examples section
    #suite = unittest.TestSuite()
    #suite.addTest(MyTests('test_equation'))
    #suite.addTest(MyTests('test_complex'))
    tests = (
             #TestAirline,
             TestEquations,
             TestAddition,
